...
using Tcc.Core.Messages.Facebook;
...
namespace Tcc.Core.Services
{
    public static class FacebookService
    {
        ...
        public static async Task<Page> ObterPaginaAsync(string pageId)
        {
            var page = await GetObjectAsync<Page>($"{GraphUrl}{pageId}?fields=name%2Cid%2Clink&access_token={AccessToken}");
